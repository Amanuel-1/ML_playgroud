  ...
  constructor(rows,cols){
      ...
      for(let i=0;i<this.rows ; i++){
          ...
          for(let j=0;j<this.cols;j++){
              this.data[i][j] =(Math.random()*2-1) ;//assign a random number to every position of the matrix
          }
      }
  }
...
/**  this function is used to convert any dimensional array into  a Matrix object */


static toMatrix(m){
  let row,col,result
  if(typeof m[0]=="number"){
    console.log(m,"this is a one dimensional number")

    row  =1
    col  =m.length
    result   = new Matrix(1,m.length)
  ...
  console.log("this is a test from matrix")
  result.print()
  return result
}
...
addBiase(m){
  ...
  if(m instanceof Matrix){
    if(m.rows==this.rows && m.cols==1){
        ...
        throw new Error("the shape of the matrices must be the same")
    }
  }
  else{
    throw new Error("the parameter m must be an instance of Matrix")
  }
}
